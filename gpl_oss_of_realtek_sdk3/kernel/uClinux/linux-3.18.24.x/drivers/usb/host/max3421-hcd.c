...
max3421_spi_thread(void *dev_id)
{
	...
	struct max3421_hcd *max3421_hcd = hcd_to_max3421(hcd);
	int i, i_worked = 1;
#if 1 /* ADD by RTK management switch */
	u8     reg_value;

	/* set half-duplex SPI mode, falling edge-active interrupt pin: */
	reg_value = spi_rd8(hcd, MAX3421_REG_PINCTL);
	printk("\n[%s][%d] MAX3421_REG_PINCTL = 0x%02x (BEFORE)",__FUNCTION__,__LINE__,reg_value);

    reg_value &= ~(BIT(MAX3421_PINCTL_INTLEVEL_BIT));
    reg_value &= ~(BIT(MAX3421_PINCTL_POSINT_BIT));
    reg_value |= (BIT(MAX3421_PINCTL_FDUPSPI_BIT));  /* Add for test*/
    printk("\n[%s][%d] MAX3421_REG_PINCTL = 0x%02x (AFTER)\n",__FUNCTION__,__LINE__,reg_value);
    spi_wr8(hcd, MAX3421_REG_PINCTL,reg_value);
#endif


#if 0 /* Mask by RTK management switch */
	...
	while (!kthread_should_stop()) {
		if (!i_worked) {
			...
			if (test_and_clear_bit(ENABLE_IRQ, &max3421_hcd->todo))
				enable_irq(spi->irq);
			//schedule();	/* Mask by RTK management switch */
			schedule_timeout(1); /*Change by SD6 for Falling-eddg trigger*/
