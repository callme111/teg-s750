...
struct device;
#if defined(CONFIG_RTK_MIPS) && defined(CONFIG_HIGHMEM)
#include <bspchip.h>
...
static inline dma_addr_t cpu_to_baddr(unsigned long cpu_addr){
    ...
    if(cpu_addr >= bsp_zone2_base){
	  DEBUG_CPU2BUS_PTK("[%s-%d]cpu_address=0x%08lx, bus_address=0x%08lx\n", __func__, __LINE__, cpu_addr, cpu_addr - bsp_zone2_phy);
	  return (cpu_addr - bsp_zone2_phy);
	}
	else{
	  return cpu_addr;
	}
...
static inline unsigned long baddr_to_cpu(dma_addr_t dma_addr){
    if(dma_addr >= bsp_zone2_phy){
	  DEBUG_CPU2BUS_PTK("[%s-%d]dma_address=0x%08x ; cpu_address=0x%08x\n", __func__, __LINE__, dma_addr, dma_addr + bsp_zone2_phy);
	  return (dma_addr + bsp_zone2_phy);
	}
	else{
	  return dma_addr;
	}
