...
int i, ret, size_internal;
unsigned dtype;

if (hub_is_superspeed(hdev)) {
    dtype = USB_DT_SS_HUB;
    size_internal = USB_DT_SS_HUB_SIZE;
} else {
    dtype = USB_DT_HUB;
    size_internal = sizeof(struct usb_hub_descriptor);
}

for (i = 0; i < 3; i++) {
    ret = usb_control_msg(hdev, usb_rcvctrlpipe(hdev, 0),
        USB_REQ_GET_DESCRIPTOR, USB_DIR_IN | USB_RT_HUB,
        dtype << 8, 0, data, size_internal,
        USB_CTRL_GET_TIMEOUT);
    if (ret >= (USB_DT_HUB_NONVAR_SIZE + 2))
        return ret;
}
...
#if 0 /*Mask by SD6*/
	//struct usb_hub *hub = hdev_to_hub(hdev); //usb_hub_to_struct_hub(hdev);
	//struct usb_device *dev = hub->ports[port1 - 1]->child;
	struct usb_device *dev = hdev->children[port1 - 1];
	printk("%s(%d): %p %d %p %d\n",__func__,__LINE__,dev, port1, data, size);
	if (dev) {
		memset(data, 0, size);
